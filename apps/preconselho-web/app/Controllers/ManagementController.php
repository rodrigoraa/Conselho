...
final class ManagementController
{
    ...
    public function editUser(Request $request, array $params): Response
    {
        ...
        $id=(int)$params['id'];
        $statement=$this->repository->db->prepare('SELECT id,nome,cpf,perfil,ativo FROM usuarios WHERE id=:id AND excluido_em IS NULL');
        ...
        $cpf=Cpf::normalize($request->body['cpf']??'');
        $role=(string)($request->body['perfil']??'');
        if($name===''||mb_strlen($name)>150||!Cpf::isValid($cpf)||!in_array($role,['ADMIN','COORDENADOR','PROFESSOR'],true))throw new HttpException(422,'VALIDATION_ERROR','Confira o nome, o CPF e o tipo de acesso.');
        ...
        try{
            ...
            $this->repository->db->prepare('UPDATE usuarios SET nome=:nome,cpf=:cpf,perfil=:perfil,alterar_senha=0,atualizado_em=CURRENT_TIMESTAMP WHERE id=:id')->execute([':nome'=>$name,':cpf'=>$cpf,':perfil'=>$role,':id'=>$id]);
            $after=['nome'=>$name,'cpf'=>Cpf::format($cpf),'perfil'=>$role];
        ...
        }catch(\PDOException$e){if($this->repository->db->inTransaction())$this->repository->db->rollBack();if($e->getCode()==='23000')throw new HttpException(422,'CPF_IN_USE','Este CPF já está sendo utilizado.');throw$e;}
