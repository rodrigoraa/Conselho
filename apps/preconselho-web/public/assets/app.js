...
const setMenuOpen=open=>{menuButton?.setAttribute('aria-expanded',String(open));mainNav?.classList.toggle('open',open);if(menuLabel)menuLabel.textContent=open?'Fechar':'Menu'};
...
if(apcSubmission){
  ...
  let previewUrl='';
  const selectedText=field=>field?.selectedOptions[0]?.value?field.selectedOptions[0].textContent.trim():'—';
  const formatFileSize=bytes=>bytes<1024?`${bytes} bytes`:bytes<1048576?`${(bytes/1024).toLocaleString('pt-BR',{maximumFractionDigits:1})} KB`:`${(bytes/1048576).toLocaleString('pt-BR',{maximumFractionDigits:1})} MB`;
  const refreshReviewContext=()=>{
    const eventCopy=apcSubmission.querySelector('[data-apc-review-event]');
    const classCopy=apcSubmission.querySelector('[data-apc-review-class]');
    if(eventCopy)eventCopy.textContent=selectedText(eventField);
    if(classCopy)classCopy.textContent=selectedText(classField);
  };
  ...
  const showFilePreview=()=>{
    const file=fileField?.files?.[0];resetFilePreview();if(!file)return;
    const nameCopy=apcSubmission.querySelector('[data-apc-review-name]');
    const sizeCopy=apcSubmission.querySelector('[data-apc-review-size]');
  ...
  eventField?.addEventListener('change',()=>{if(stage)stage.value='';if(year)year.value='';if(classField)classField.value='';refreshStages();refreshReviewContext()});
  stage?.addEventListener('change',()=>{if(year)year.value='';if(classField)classField.value='';refreshSeries();refreshReviewContext()});
  year?.addEventListener('change',()=>{if(classField)classField.value='';refreshClasses();refreshReviewContext()});
  classField?.addEventListener('change',()=>{const selected=classField.selectedOptions[0];if(classesHelp&&selected?.value)classesHelp.textContent=`Turma selecionada: ${selected.textContent}`;refreshReviewContext()});
  fileField?.addEventListener('change',showFilePreview);
  apcSubmission.querySelector('[data-apc-change-file]')?.addEventListener('click',()=>fileField?.click());
  ...
  resetFilePreview();refreshStages();
}
...
const curriculumPlan=document.querySelector('[data-curriculum-plan]');
if(curriculumPlan){
  const stage=curriculumPlan.querySelector('[data-curriculum-stage]'),year=curriculumPlan.querySelector('[data-curriculum-year]'),componentSelect=curriculumPlan.querySelector('[data-ability-component]'),query=curriculumPlan.querySelector('[data-ability-query]'),results=curriculumPlan.querySelector('[data-ability-results]'),status=curriculumPlan.querySelector('[data-ability-status]'),componentStatus=curriculumPlan.querySelector('[data-components-status]'),selected=curriculumPlan.querySelector('[data-selected-abilities]'),empty=curriculumPlan.querySelector('[data-no-abilities]');
  const checkedComponents=()=>[...curriculumPlan.querySelectorAll('[data-curriculum-components] input:checked')];
  const refreshEmpty=()=>{if(empty)empty.hidden=!!selected?.querySelector('[data-ability-id]')};
  const removeAbility=button=>{button.closest('[data-ability-id]')?.remove();refreshEmpty()};
  curriculumPlan.querySelectorAll('[data-remove-ability]').forEach(button=>button.addEventListener('click',()=>removeAbility(button)));
  const refreshComponents=()=>{const selectedValue=componentSelect?.value||'';let visible=0;curriculumPlan.querySelectorAll('[data-curriculum-components]>label').forEach(label=>{const show=!!stage?.value&&label.dataset.stage===stage.value;label.hidden=!show;if(show)visible++;if(!show)label.querySelector('input').checked=false});if(componentStatus)componentStatus.textContent=stage?.value?`${visible} componente(s) disponível(is) para a etapa selecionada.`:'Selecione a etapa para exibir seus componentes.';if(componentSelect){componentSelect.replaceChildren(new Option('Selecione',''));checkedComponents().forEach(input=>componentSelect.add(new Option(input.dataset.componentName||input.value,input.value)));if([...componentSelect.options].some(option=>option.value===selectedValue))componentSelect.value=selectedValue;}const activeIds=new Set(checkedComponents().map(input=>input.value));selected?.querySelectorAll('[data-ability-id]').forEach(item=>{if(!activeIds.has(item.dataset.componentId))item.remove()});refreshEmpty()};
  const refreshStage=(changed=false)=>{year?.querySelectorAll('option[data-stage]').forEach(option=>{option.hidden=!!stage?.value&&option.dataset.stage!==stage.value;if(option.hidden&&option.selected)year.value=''});if(changed){results?.replaceChildren();if(status)status.textContent='Etapa alterada. Selecione o ano/série e um componente desta etapa.';}refreshComponents()};
  stage?.addEventListener('change',()=>refreshStage(true));year?.addEventListener('change',()=>{results?.replaceChildren();if(status)status.textContent='Ano/série alterado. Faça uma nova busca.'});curriculumPlan.querySelectorAll('[data-curriculum-components] input').forEach(input=>input.addEventListener('change',refreshComponents));refreshStage();
  const addAbility=ability=>{if(selected?.querySelector(`[data-ability-id="${ability.id}"]`))return;const article=document.createElement('article');article.className='selected-ability';article.dataset.abilityId=String(ability.id);article.dataset.componentId=String(ability.componente_id);const hidden=document.createElement('input');hidden.type='hidden';hidden.name='habilidades[]';hidden.value=String(ability.id);const copy=document.createElement('div'),badge=document.createElement('span'),code=document.createElement('strong'),description=document.createElement('p'),remove=document.createElement('button');badge.className='badge';badge.textContent=ability.componente_nome;code.textContent=ability.codigo||'Sem código';description.textContent=ability.descricao;remove.type='button';remove.className='button small-button';remove.dataset.removeAbility='';remove.textContent='Remover';remove.addEventListener('click',()=>removeAbility(remove));copy.append(badge,code,description);article.append(hidden,copy,remove);selected?.append(article);refreshEmpty()};
  curriculumPlan.querySelector('[data-search-abilities]')?.addEventListener('click',async()=>{if(!stage?.value||!year?.value||!componentSelect?.value){if(status)status.textContent='Selecione etapa, ano/série e um componente marcado.';return}if(status)status.textContent='Buscando no catálogo local…';results?.replaceChildren();const params=new URLSearchParams({etapa:stage.value,ano:year.value,componente:componentSelect.value,q:query?.value||''});try{const response=await fetch('/apc/habilidades?'+params.toString(),{headers:{Accept:'application/json'}});if(!response.ok)throw new Error('search');const data=await response.json();if(status)status.textContent=`${data.resultados.length} resultado(s), limite de ${data.limite}.`;data.resultados.forEach(ability=>{const article=document.createElement('article'),copy=document.createElement('div'),code=document.createElement('strong'),description=document.createElement('p'),meta=document.createElement('small'),button=document.createElement('button');article.className='ability-result';code.textContent=ability.codigo||'Sem código';description.textContent=ability.descricao;meta.textContent=[ability.unidade_tematica,ability.objeto_conhecimento,ability.origem].filter(Boolean).join(' · ');button.type='button';button.className='button small-button';button.textContent='Adicionar';button.addEventListener('click',()=>addAbility(ability));copy.append(code,description,meta);article.append(copy,button);results?.append(article)})}catch(error){if(status)status.textContent='Não foi possível consultar o catálogo. Tente novamente.'}});
  query?.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();curriculumPlan.querySelector('[data-search-abilities]')?.click()}});refreshEmpty();
}
