...
if(collectiveDocument){
  ...
  classSections.forEach(section=>{
    ...
    const csrf=section.querySelector('input[name="_csrf"]')?.value||'';
    ...
    const save=()=>{if(!textarea)return Promise.resolve(true);if(inFlight)return inFlight.then(saved=>dirty?save():saved);if(!dirty)return Promise.resolve(true);dirty=false;if(status)status.textContent='Salvando texto coletivo…';const body=new FormData();body.append('_csrf',csrf);body.append('conteudo',textarea.value);body.append('versao',textarea.dataset.version||'0');inFlight=fetch(textarea.dataset.autosaveUrl,{method:'POST',body,headers:{'X-Requested-With':'XMLHttpRequest'}}).then(async response=>{if(!response.ok){if(response.status===409)throw new Error('conflict');if(response.status===422)throw new Error('protected');throw new Error('save')}return response.json()}).then(data=>{textarea.dataset.version=String(data.version);if(!dirty)textarea.dataset.dirty='0';refreshUnsavedState();section.classList.remove('save-conflict');if(status)status.textContent=`✓ Texto salvo às ${data.saved_at}.`;return true}).catch(error=>{dirty=true;textarea.dataset.dirty='1';refreshUnsavedState();section.classList.add('save-conflict');if(status)status.textContent=error.message==='conflict'?'Outro professor atualizou esta turma. Recarregue a página antes de continuar.':error.message==='protected'?'O texto já salvo não pode ser apagado ou substituído. Use Ctrl+Z ou recarregue a página e apenas acrescente conteúdo.':'Não foi possível salvar. O sistema tentará novamente quando você escrever.';return false}).finally(()=>{inFlight=null});return inFlight};
    textarea?.addEventListener('input',()=>{dirty=true;textarea.dataset.dirty='1';refreshUnsavedState();section.classList.remove('save-conflict');render();if(status)status.textContent='Alterações aguardando salvamento…';if(timer)clearTimeout(timer);timer=setTimeout(save,1200)});
