...
<div class="admin-workspace">
    ...
    <section class="card admin-section admin-section-block binding-section" id="vinculos">
        ...
        <details class="admin-collapsible admin-create-panel">
            ...
            <div class="admin-collapsible-body">
                <form method="post" action="/admin/vinculos" id="binding-form">
                    ...
                    <div class="binding-step"><span class="step-number">2</span><div><fieldset class="shift-choice-fieldset"><legend>Turnos</legend><div class="shift-choice-list"><label class="choice-card"><input type="checkbox" name="turnos[]" value="MATUTINO" checked><span><strong>Matutino</strong></span></label><label class="choice-card"><input type="checkbox" name="turnos[]" value="VESPERTINO"><span><strong>Vespertino</strong></span></label></div><p class="helper">Marque os dois quando o professor lecionar no matutino e no vespertino.</p></fieldset></div></div>
        ...
        <?php if(!$bindings):?><div class="empty-state compact"><p>Nenhum vínculo cadastrado.</p></div><?php else:?><div class="binding-professor-list" id="binding-management-list">
            <?php foreach($bindingsByProfessor as $professorId=>$group):$rows=$group['rows'];$classCount=count(array_unique(array_column($rows,'turma_externa_id')));$shiftCount=count(array_unique(array_column($rows,'turno')));$activeCount=count(array_filter($rows,static fn(array $row):bool=>(bool)$row['ativo']));?>
                <article class="binding-professor-group">
                    <details class="binding-professor-details">
                        <summary>
                            <span class="professor-identity"><span class="avatar" aria-hidden="true"><?=e(mb_strtoupper(mb_substr($group['nome'],0,1)))?></span><span><strong><?=e($group['nome'])?></strong><small><?=count($rows)?> vínculo(s) · <?=$classCount?> turma(s) · <?=$shiftCount===2?'Matutino e Vespertino':($rows[0]['turno']==='MATUTINO'?'Matutino':'Vespertino')?></small></span></span>
                            <span class="badge status-<?=$activeCount?'aprovado':'pendente'?>"><?=$activeCount?> ativo(s)</span>
                        </summary>
                        <div class="binding-group-body">
                            <div class="binding-group-actions"><p class="helper">A exclusão geral remove todas as turmas e turnos deste professor. Documentos de conselhos já criados permanecem preservados.</p><form method="post" action="/admin/professores/<?=e($professorId)?>/vinculos/excluir"><input type="hidden" name="_csrf" value="<?=e(Csrf::token())?>"><button class="danger" data-confirm="Excluir todos os <?=count($rows)?> vínculos de <?=e($group['nome'])?>? Esta ação removerá todas as turmas dos turnos matutino e vespertino desse professor.">Excluir todos os vínculos</button></form></div>
