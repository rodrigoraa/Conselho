...
foreach($classes as$class){$classText=trim((string)$class['conteudo']);if($classText!=='')$finalSegments[]=$class['nome_conselho'].': '.$classText;}
...
<article class="paper-document collective-paper collective-editor-paper" data-document-editor>
    ...
    <?php foreach($classes as$index=>$class):$content=(string)$class['conteudo'];$classLabel=(string)$class['nome_conselho'];$mine=(bool)$class['minha_turma'];$finished=(bool)$class['meu_finalizado'];$editable=$canWrite&&($isPrivilegedEditor||($mine&&!$finished));$classConclusions=$conclusions[(int)$class['id']]??[];$classSegments=$class['segmentos']??[];$segmentCount=count($classSegments);$classLock=$class['bloqueio']??null;$lockedByOther=$classLock&&(int)$classLock['usuario_id']!==(int)($_SESSION['user']['id']??0);?>
        <details id="turma-<?=e($class['id'])?>" class="paper-class-section shared-class-section <?=trim($content)===''?'is-empty':''?> <?=$mine?'is-mine':'is-readonly'?> <?=$finished?'is-finalized':''?>" data-collective-class data-mine="<?=$mine?'1':'0'?>" data-class-id="<?=e($class['id'])?>" data-class-name="<?=e($classLabel)?>">
            <summary class="paper-section-heading"><span><?=e($index+1)?></span><div><h3><?=e($classLabel)?></h3><p><?=$mine?'Você está vinculado':($isPrivilegedEditor?'Acesso da gestão':'Consulta')?> · <span data-contribution-count><?=e($segmentCount)?></span> trecho(s) com autoria</p></div><span class="class-responsibility badge status-<?=$finished?'aprovado':($editable?'rascunho':'pendente')?>"><?=$finished?'Você finalizou':($editable?'Edição disponível':'Somente leitura')?></span><span class="class-expand-label" aria-hidden="true">Abrir</span></summary>
