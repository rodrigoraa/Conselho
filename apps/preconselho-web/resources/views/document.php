...
<?php if($isTeacher):?><aside class="document-instructions"><div><strong>Como funciona</strong><p><?=$collaborationEnabled?'Abra uma de suas turmas e escreva junto com os demais professores em tempo real. Os cursores e participantes conectados aparecem no editor, mas cada professor continua podendo alterar somente os próprios trechos.':'Abra uma de suas turmas e clique no texto para começar. Enquanto você estiver digitando, a turma ficará temporariamente bloqueada para os demais. Você pode alterar somente os próprios trechos.'?> Depois de finalizar, dependerá de uma nova liberação.</p></div><div class="document-completion"><span><strong data-my-finished><?=e($myFinished)?></strong> de <?=e($myClasses)?> turmas finalizadas</span><progress max="<?=max(1,$myClasses)?>" value="<?=e($myFinished)?>"><?=e($myFinished)?></progress><small>Seu acompanhamento individual</small></div></aside><?php else:?><aside class="document-instructions"><div><strong>Preenchimento e acompanhamento</strong><p><?=$collaborationEnabled?'Coordenação e administração podem acompanhar e editar qualquer turma ao vivo, inclusive enquanto os professores estão escrevendo.':'Coordenação e administração podem escrever e alterar qualquer trecho de qualquer turma. Ao iniciar uma edição, a turma fica temporariamente reservada para esse usuário, evitando alterações simultâneas.'?></p></div><div class="document-completion"><span><strong><?=e($totalFinished)?></strong> de <?=count($allContributions)?> finalizações</span><progress max="<?=max(1,count($allContributions))?>" value="<?=e($totalFinished)?>"><?=e($totalFinished)?></progress><small>Coordenação e administração</small></div></aside><?php endif;?>
    ...
    <?php foreach($classes as$index=>$class):$content=(string)$class['conteudo'];$classLabel=(string)$class['nome_conselho'];$mine=(bool)$class['minha_turma'];$finished=(bool)$class['meu_finalizado'];$editable=$canWrite&&($isPrivilegedEditor||($mine&&!$finished));$classConclusions=$conclusions[(int)$class['id']]??[];$progress=$classTracking[(int)$class['id']];$classComplete=(bool)$progress['completa'];$classHasPending=(int)$progress['total']>0&&!$classComplete;$classPendingNames=array_map(static fn(array$item):string=>(string)$item['professor_nome'],$progress['pendentes']);$classSegments=$class['segmentos']??[];$segmentCount=count($classSegments);$classLock=$class['bloqueio']??null;$lockedByOther=$classLock&&(int)$classLock['usuario_id']!==(int)($_SESSION['user']['id']??0);$collaborationToken=$editable&&$collaborationEnabled?CollaborationToken::issue(['sub'=>$collaborationUserId,'name'=>$collaborationUserName,'role'=>$currentRole,'period'=>(int)$period,'class'=>(int)$class['id']],$collaborationSecret,Env::int('COLLABORATION_TOKEN_TTL',28800)):'';?>
        <details id="turma-<?=e($class['id'])?>" class="paper-class-section shared-class-section <?=trim($content)===''?'is-empty':''?> <?=$mine?'is-mine':'is-readonly'?> <?=$finished?'is-finalized':''?> <?=$classComplete?'is-complete':($classHasPending?'has-pending':'has-no-assignees')?>" data-collective-class data-mine="<?=$mine?'1':'0'?>" data-class-id="<?=e($class['id'])?>" data-class-name="<?=e($classLabel)?>">
            ...
            <div class="shared-class-body">
                <?php if($editable):?>
                    <article class="shared-text-editor"><header><div><strong>Texto coletivo da turma</strong><small><?=$collaborationToken!==''?'Edição simultânea ativada: você verá ao vivo quem está nesta turma e o que está sendo escrito. '.($isPrivilegedEditor?'Você pode corrigir qualquer trecho.':'Os trechos dos demais professores continuam protegidos.') :($isPrivilegedEditor?'Você pode escrever, corrigir ou apagar qualquer trecho desta turma.':'Você pode escrever em qualquer ponto e alterar somente os trechos de sua autoria. O texto dos demais usuários fica protegido.')?></small></div><span class="badge <?=$collaborationToken!==''?'status-enviado':($lockedByOther?'status-pendente':'status-rascunho')?>" data-lock-badge><?=$collaborationToken!==''?'Tempo real':($lockedByOther?'Em edição':'Disponível')?></span></header><label><span class="sr-only">Texto coletivo da turma <?=e($classLabel)?></span><textarea maxlength="60000" rows="14" readonly data-shared-content data-version="<?=e($class['versao'])?>" data-lock-url="/documentos/<?=e($period)?>/turmas/<?=e($class['id'])?>/bloqueio" data-release-lock-url="/documentos/<?=e($period)?>/turmas/<?=e($class['id'])?>/bloqueio/liberar" data-autosave-url="/documentos/<?=e($period)?>/turmas/<?=e($class['id'])?>/autosave"<?php if($collaborationToken!==''):?> data-collaboration-token="<?=e($collaborationToken)?>" data-collaboration-url="<?=e($collaborationUrl)?>" data-collaboration-document="council:<?=e($period)?>:<?=e($class['id'])?>" data-collaboration-user="<?=e($collaborationUserName)?>" data-collaboration-color="<?=e($collaborationColor)?>"<?php endif;?> placeholder="Comece o texto coletivo desta turma."><?=e($content)?></textarea></label><input type="hidden" value="<?=e(Csrf::token())?>" data-class-csrf><small class="shared-editor-status" data-shared-save-status aria-live="polite"><?=$collaborationToken!==''?'Abra esta turma para conectar ao editor em tempo real.':($lockedByOther?e($classLock['usuario_nome_snapshot']).' está editando esta turma.':'Clique no texto para iniciar a edição.')?></small></article>
