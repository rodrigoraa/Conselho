...
final class SubmissionService
{
    ...
    public function tracking():array
    {
        ...
        foreach($events as$event){
            ...
            foreach($eventRequirements as$requirement){
                $historicalBindingChanged=$scheduleState!==null&&(!$this->access->isActiveTeacher((int)$requirement['professor_usuario_id'])||!$this->access->hasActiveClassBinding((int)$requirement['professor_usuario_id'],(int)$requirement['turma_id_externo'],(int)$event['ano_letivo'],(string)$requirement['turno']));
                if($scheduleState!==null)$requirement=$this->requirementPresentation($requirement)+['vinculo_atual_alterado'=>$historicalBindingChanged];
    ...
    public function submit(array$input,array$file,array$user,string$ip,string$userAgent):int
    {
        ...
        if($this->schedules!==null){$scheduleState=$this->schedules->ensureEvent($event);if($scheduleState['status']==='DIA_REFERENCIA_NAO_CONFIGURADO')throw new HttpException(422,'APC_EVENT_REFERENCE_DAY','A coordenação precisa definir o dia da grade de referência deste evento.');if($scheduleState['status']==='NAO_CONFIGURADA')throw new HttpException(422,'APC_SCHEDULE_NOT_CONFIGURED','Não há grade de horários configurada para o ano, turno e data deste evento.');if($scheduleState['status']==='LEGADO')throw new HttpException(422,'APC_LEGACY_EVENT','Este evento é histórico e não aceita novas obrigações inferidas.');$allowed=false;foreach($scheduleState['requirements']as$obligation)if((int)$obligation['professor_usuario_id']===$userId&&(int)$obligation['turma_id_externo']===(int)$classId){$allowed=true;break;}if(!$allowed)throw new HttpException(403,'APC_SCHEDULE_FORBIDDEN','Você não possui aula nesta turma na data oficial da APC.');}
