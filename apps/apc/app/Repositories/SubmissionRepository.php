...
final class SubmissionRepository
{
    ...
    public function existing(int$eventId,int$userId,string$stage,string$year):?array
    {
        $statement=$this->db->prepare('SELECT * FROM apc_envios WHERE evento_id=:evento AND professor_usuario_id=:usuario AND etapa=:etapa AND ano_serie=:serie');$statement->execute([':evento'=>$eventId,':usuario'=>$userId,':etapa'=>$stage,':serie'=>$year]);return$statement->fetch()?:null;
    }

    public function save(?int$id,array$data):int
    {
        $parameters=[':evento'=>$data['evento_id'],':bimestre'=>$data['bimestre_id'],':usuario'=>$data['professor_usuario_id'],':professor'=>$data['professor_nome_snapshot'],':etapa'=>$data['etapa'],':serie'=>$data['ano_serie'],':original'=>$data['nome_original'],':armazenado'=>$data['nome_armazenado'],':mime'=>$data['mime_type'],':tamanho'=>$data['tamanho_bytes'],':sha'=>$data['sha256'],':caminho'=>$data['caminho_relativo'],':atrasado'=>$data['atrasado'],':dias'=>$data['dias_atraso'],':enviado'=>$data['enviado_em']];
        if($id===null){$statement=$this->db->prepare('INSERT INTO apc_envios(evento_id,bimestre_id,professor_usuario_id,professor_nome_snapshot,etapa,ano_serie,nome_original,nome_armazenado,mime_type,tamanho_bytes,sha256,caminho_relativo,atrasado,dias_atraso,enviado_em)VALUES(:evento,:bimestre,:usuario,:professor,:etapa,:serie,:original,:armazenado,:mime,:tamanho,:sha,:caminho,:atrasado,:dias,:enviado)');$statement->execute($parameters);return(int)$this->db->lastInsertId();}
    ...
    {
        return"SELECT s.*,e.data evento_data,e.titulo evento_titulo,e.tipo evento_tipo,e.ano_letivo,b.numero bimestre_numero,b.data_inicio bimestre_inicio,b.data_fim bimestre_fim,(SELECT GROUP_CONCAT(t.turma_nome_snapshot,' • ') FROM apc_envio_turmas t WHERE t.envio_id=s.id ORDER BY t.turma_nome_snapshot) turmas FROM apc_envios s JOIN apc_eventos e ON e.id=s.evento_id JOIN apc_bimestres b ON b.id=s.bimestre_id";
    }
}
