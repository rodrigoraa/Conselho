...
use Apc\Services\{AttachmentService,AuthorizationService,CalendarImporter,CurriculumImporter,CurriculumService,DeliveryService,EventService,EventWindow,PlanService,SettingsService,SubmissionService,SubmissionWindow};
...
final class Module
{
    ...
    public function __construct(PDO $apcDb,PDO $mainDb,SecretariaApiClient $api,string $root)
    {
        ...
        $storage=StorageFactory::fromEnvironment($root);$planService=new PlanService($plans,$events,$access,$audit,$authorization,$api,$curriculum,$eventWindow);$curriculumService=new CurriculumService($curriculum,$audit);$curriculumImporter=new CurriculumImporter($curriculum,$audit,$root.'/apps/apc/resources/curriculo');$calendarImporter=new CalendarImporter($events,$audit,$root.'/apps/apc/resources/calendario/eventos_ee_sao_jose_2026.csv');$deliveryService=new DeliveryService($plans,$deliveryRepository,$settings,$audit,$authorization,$api,$eventWindow);$attachmentService=AttachmentService::fromEnvironment($deliveryRepository,$audit,$authorization,$root,$eventWindow,$storage);$eventService=new EventService($events,$audit);$settingsService=new SettingsService($settings,$audit,$apcDb);
