...
{
    public function __construct(private readonly EventRepository $events,private readonly PlanRepository $plans,private readonly View $view) {}
    public function index(Request$request):Response
    {
        $now=new \DateTimeImmutable('today');$year=$this->integer($request->query['ano']??$now->format('Y'),2000,2100,'ano');$month=$this->integer($request->query['mes']??$now->format('n'),1,12,'mês');$events=$this->events->month($year,$month);$years=$this->events->years();if(!in_array($year,$years,true))$years[]=$year;rsort($years);return new Response($this->view->render('calendar',compact('year','month','events','years','now')+['title'=>'Calendário de APCs']));
    }
    public function show(Request$request,array$params):Response
    {
        $event=$this->events->find((int)$params['id'])??throw new HttpException(404,'APC_EVENT_NOT_FOUND','Evento APC não encontrado.');$user=$_SESSION['user'];$summary=$this->plans->eventSummary((int)$event['id'],(int)$user['id'],(string)$user['perfil']);$teacherPlans=$user['perfil']==='PROFESSOR'?$this->plans->eventPlansForTeacher((int)$event['id'],(int)$user['id']):[];return new Response($this->view->render('event',compact('event','summary','teacherPlans')+['title'=>'Evento APC']));
    }
