...
final class DashboardController
{
    ...
    {
        $user=$_SESSION['user'];$filters=$this->filters($request);$plans=$this->plans->list((int)$user['id'],(string)$user['perfil'],$filters);$filterOptions=$this->plans->list((int)$user['id'],(string)$user['perfil'],[]);$metrics=$this->plans->dashboardMetrics((int)$user['id'],(string)$user['perfil']);$events=$this->events->active();$decorated=array_map(fn(array$event):array=>$event+['window'=>$this->window()->describe($event)],$events);$openEvents=array_values(array_filter($decorated,static fn(array$event):bool=>$event['window']['is_open']));$upcomingEvents=array_slice(array_values(array_filter($decorated,static fn(array$event):bool=>in_array($event['window']['state'],['AGUARDANDO','AGUARDANDO_LIBERACAO'],true))),0,5);$managedEvents=$user['perfil']==='PROFESSOR'?[]:array_slice(array_values(array_filter($decorated,static fn(array$event):bool=>!in_array($event['window']['state'],['ENCERRADA','CANCELADA'],true))),0,8);$classes=$this->access->classesFor((int)$user['id'],(string)$user['perfil']);
        return new Response($this->view->render('dashboard',compact('plans','filterOptions','metrics','events','openEvents','upcomingEvents','managedEvents','classes','filters')+['title'=>'APCs']));
    }
