...
final class ReportServiceTest extends TestCase
{
    ...
    public function testAberturaDoPeriodoGeraDocumentoPorTurmaSemRelatorioDeDisciplina():void
    {$this->db->exec("INSERT INTO periodos_pre_conselho(id,nome,ano_letivo,etapa,data_inicio,data_fim,status,criado_por,turno)VALUES(2,'Novo período',2026,'2º','2020-01-01','2099-12-31','RASCUNHO',1,'MATUTINO')");(new PeriodService(new AppRepository($this->db)))->open(2,1,'127.0.0.1','test');self::assertSame(0,(int)$this->db->query('SELECT COUNT(*) FROM relatorios_pre_conselho WHERE periodo_id=2')->fetchColumn());self::assertSame(1,(int)$this->db->query('SELECT COUNT(*) FROM documento_turmas WHERE periodo_id=2')->fetchColumn());self::assertSame(1,(int)$this->db->query('SELECT COUNT(*) FROM documento_turma_professores c JOIN documento_turmas dt ON dt.id=c.documento_turma_id WHERE dt.periodo_id=2')->fetchColumn());}
