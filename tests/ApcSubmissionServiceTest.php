...
final class ApcSubmissionServiceTest extends ApcTestCase
{
    ...
    public function testReleasedApcFromAnotherSemesterRemainsOpenUntilCoordinationSuspendsIt():void
    {
        [$db,$repository]=$this->service('2026-08-12');$db->exec("INSERT INTO apc_eventos(id,ano_letivo,data,titulo,tipo,origem,descricao,status,criado_por,disponibilizado_em,disponibilizado_por)VALUES(2,2026,'2026-03-15','APC do primeiro semestre','OUTRO','ESCOLA','','ATIVO',1,'2026-08-12 08:00:00',2)");$service=$this->serviceFor($db,$repository,'2026-08-12');$id=$service->submit($this->input(eventId:2),$this->file('primeiro-semestre.png'),['id'=>3,'nome'=>'Professor Um','perfil'=>'PROFESSOR'],'127.0.0.1','phpunit');self::assertSame(150,(int)$repository->find($id)['dias_atraso']);$db->exec('UPDATE apc_eventos SET disponibilizado_em=NULL,disponibilizado_por=NULL WHERE id=2');try{$service->submit($this->input(eventId:2),$this->file('suspensa.png'),['id'=>3,'nome'=>'Professor Um','perfil'=>'PROFESSOR'],'127.0.0.1','phpunit');self::fail('APC suspensa deveria recusar novos envios.');}catch(HttpException$exception){self::assertSame('APC_EVENT_NOT_RELEASED',$exception->errorCode);}
    }
