...
final class ApcSubmissionServiceTest extends ApcTestCase
{
    ...
    public function testProfessorUploadsUsingCouncilBindingAndLateReplacementIsVisible():void
    {
        [$db,$repository,$service]=$this->service('2026-08-11');$teacher=['id'=>3,'nome'=>'Professor Um','perfil'=>'PROFESSOR'];$id=$service->submit($this->input(),$this->file('modelo.png'),$teacher,'127.0.0.1','phpunit');$first=$repository->find($id);self::assertSame(0,(int)$first['atrasado']);self::assertSame('7º A',$first['turmas']);self::assertFileExists($firstPath=$service->file($id,$teacher)['caminho_absoluto']);$lateService=$this->serviceFor($db,$repository,'2026-08-20');$sameId=$lateService->submit($this->input(),$this->file('modelo-corrigido.png'),$teacher,'127.0.0.1','phpunit');$updated=$repository->find($sameId);self::assertSame($id,$sameId);self::assertSame(1,(int)$updated['atrasado']);self::assertSame(5,(int)$updated['dias_atraso']);self::assertSame('modelo-corrigido.png',$updated['nome_original']);self::assertFileDoesNotExist($firstPath);self::assertSame(['ANEXAR_ARQUIVO_APC','SUBSTITUIR_ARQUIVO_APC'],$db->query("SELECT acao FROM apc_auditoria WHERE entidade='apc_envios' ORDER BY id")->fetchAll(\PDO::FETCH_COLUMN));
    }
    ...
    private function input(string$stage='EF_AF',string$year='EF7'):array{return['evento_id'=>1,'etapa'=>$stage,'ano_serie'=>$year];}
