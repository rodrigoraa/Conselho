...
final class ApcSubmissionPreviewTest extends ApcTestCase
{
    ...
    {
        $controller=$this->controller('application/pdf','pdf','%PDF-1.4 preview');$_SESSION['user']=['id'=>3,'nome'=>'Professor Um','perfil'=>'PROFESSOR'];$_SERVER['REQUEST_URI']='/apc/envios/1/visualizar';$preview=$controller->preview(new Request('GET','/apc/envios/1/visualizar',[],[],[]),['id'=>1]);self::assertStringContainsString('Conteúdo da APC',$preview->body);self::assertStringContainsString('/apc/envios/1/conteudo',$preview->body);self::assertStringContainsString('Visualizar envio',$preview->body);$content=$controller->content(new Request('GET','/apc/envios/1/conteudo',[],[],[]),['id'=>1]);self::assertSame('%PDF-1.4 preview',$content->body);self::assertStringStartsWith('inline;',$content->headers['Content-Disposition']);self::assertSame('SAMEORIGIN',$content->headers['X-Frame-Options']);$_SESSION['user']=['id'=>2,'nome'=>'Coordenação','perfil'=>'COORDENADOR'];$coordination=$controller->preview(new Request('GET','/apc/envios/1/visualizar',[],[],[]),['id'=>1]);self::assertStringContainsString('Professor Um',$coordination->body);
    }

    public function testAnotherTeacherCannotPreviewAndUnsupportedDocumentUsesSafeFallback():void
    {
        $controller=$this->controller('application/vnd.openxmlformats-officedocument.wordprocessingml.document','docx','documento');$_SESSION['user']=['id'=>4,'nome'=>'Professor Dois','perfil'=>'PROFESSOR'];try{$controller->preview(new Request('GET','/apc/envios/1/visualizar',[],[],[]),['id'=>1]);self::fail('Outro professor não deveria visualizar este arquivo.');}catch(HttpException$exception){self::assertSame(403,$exception->status);}$_SESSION['user']=['id'=>3,'nome'=>'Professor Um','perfil'=>'PROFESSOR'];$preview=$controller->preview(new Request('GET','/apc/envios/1/visualizar',[],[],[]),['id'=>1]);self::assertStringContainsString('Visualização direta indisponível',$preview->body);try{$controller->content(new Request('GET','/apc/envios/1/conteudo',[],[],[]),['id'=>1]);self::fail('DOCX não deveria ser servido como conteúdo inline.');}catch(HttpException$exception){self::assertSame(415,$exception->status);self::assertSame('APC_PREVIEW_UNSUPPORTED',$exception->errorCode);}
    }
